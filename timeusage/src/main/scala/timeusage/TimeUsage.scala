...
object TimeUsage extends TimeUsageInterface {
  ...
  def dfSchema(fields: List[String]): StructType = {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val primaryNeedsColumns = primaryNeeds.flatMap(c => columnNames.filter(_.startsWith(c)).map(col))
    val workColumns = work.flatMap(c => columnNames.filter(_.startsWith(c)).map(col))
    val leisureColumns = leisure.flatMap(c => columnNames.filter(_.startsWith(c)).map(col))
  ...
  def timeUsageSummary(
    ...
    val workingStatusProjection: Column = when(df("telfs") < 3, "working")
    ...
    val primaryNeedsProjection: Column = (primaryNeedsColumns.map(c => df(c.toString())).reduce(_ + _) / 60.0).as("primaryNeeds")
    val workProjection: Column = (workColumns.map(c => df(c.toString())).reduce(_ + _) / 60.0).as("work")
    val otherProjection: Column = (otherColumns.map(c => df(c.toString())).reduce(_ + _) / 60.0).as("other")
    df
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed.groupByKey(s => (s.age, s.sex, s.working))
